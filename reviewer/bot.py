...
from reviewer.github_client import get_pull_request, get_pr_diff, post_review, post_error_comment
...
def main():
    ...
    # Get the PR first so we can post error comments if anything goes wrong downstream
    pr = get_pull_request()
    ...
    try:
        ...
        if total_lines > MAX_DIFF_LINES:
            ...
            pr.create_issue_comment(
                f"⚠️ **AI Review skipped** — this PR is too large for automated review "
        ...
        # Don't fail silently — let the PR author know something went wrong
        print(f"❌ Review failed: {type(e).__name__}: {e}")
        post_error_comment(pr, e)
        raise  # Re-raise so the GitHub Actions step still shows as failed
